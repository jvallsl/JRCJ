		...
		<section>
				...
				<?php
					...
					if(!isset($numcodparte)){
						...
						$sql = "SELECT UsuarioId, Username, Contrasena, Email, Registro, Experiencia, Activo, administrador
		                  		FROM usuario";
						...
						echo "<tr><td>Numeración</td><td>Usuario</td><td>Contrasena</td><td>Email</td><td>Registro</td><td>Experiencia</td><td>Activo</td><td>Administrador</td></tr>";
						while ($fila = mysql_fetch_array($resultado)){
							echo"				
								<tr class='producto'>
		    						<th><input name='UsuarioId[$i]' value='".$fila[0]."' readonly='readonly'/></th>
		    						    					
		    						<th><input name='Username[$i]' value='".$fila[1]."' readonly='readonly'/></th>

		    						<th><input name='Contrasena[$i]' value='".$fila[2]."'/></th>
		    						
		    						<th><input name='Email[$i]' value='".$fila[3]."' readonly='readonly'/></th>

		    						<th><input name='Registro[$i]' value='".$fila[4]."' readonly='readonly'/></th>

		    						<th><input name='Experiencia[$i]' value='".$fila[5]."' readonly='readonly'/></th>

									<th><input name='Activo[$i]' value='".$fila[6]."'/></th>

		    						<th><input name='administrador[$i]' value='".$fila[7]."'/></th>
					...
					foreach ($_POST['seleccion'] as $indice => $valor){
						...
						if($opcion == "modifica"){	
							//realizem el update 
							$sql1 = "UPDATE usuario SET Username='".$_POST['Username'][$indice]."', Email='".$_POST['Email'][$indice]."', Experiencia='".$_POST['Experiencia'][$indice]."', Contrasena='".$_POST['Contrasena'][$indice]."', Activo='".$_POST['Activo'][$indice]."', administrador='".$_POST['administrador'][$indice]."'
								
								WHERE UsuarioId='".$_POST['UsuarioId'][$indice]."'";
							...
							if (! $result){
								...
								echo '<meta http-equiv="Refresh" content="0; URL=admon_user.php">';
							}
						}			
					}	
				?>
